...
type Props = {
  visible: boolean;
  onClose: () => void;
  /** Primary action — hide (main list) or unhide (hidden list). */
  onHide: () => void;
  /** Label for the primary action (default "Hide chat"). */
  primaryLabel?: string;
  /** Delete the conversation for the current user. */
  onDelete: () => void;
  /** Block the other participant. */
  onBlock: () => void;
  /** Display name / handle shown in the sheet title. */
  subject?: string;
};

/**
 * Long-press actions for a conversation row on the Messages list:
 * Hide chat, Delete chat, Block user.
 */
export function ConversationActionsSheet({
  visible,
  onClose,
  onHide,
  primaryLabel = 'Hide chat',
  onDelete,
  onBlock,
  subject,
}: Props) {
